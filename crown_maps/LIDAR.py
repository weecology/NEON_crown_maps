...
def fetch_lidar_filename(row, dirname):
    ...
    if os.path.exists(direct_filename):
        ...
        print("Filename: %s does not exist, searching within %s" %
              (direct_filename, dirname))
...
def postprocess_CHM(shapefile, CHM, min_height):
    ...
    boxes[["left", "bottom", "right", "top"]] = boxes[["left", "bottom", "right",
                                                       "top"]].astype(float)

    draped_boxes = rasterstats.zonal_stats(shapefile,
                                           CHM,
                                           stats="mean",
