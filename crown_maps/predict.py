...
def run_non_max_suppression(predicted_boxes, iou_threshold=0.15):
    ...
    with tf.Session() as sess:
        print("{} predictions in overlapping windows, applying non-max supression".format(
        ...
        mosaic_df = pd.DataFrame(
            image_detections, columns=["xmin", "ymin", "xmax", "ymax", "score", "label"])
...
def predict_tiles(model,
    ...
    for index, tfrecord in enumerate(records):
        ...
        #Run predictions
        boxes = predict_tile(model=model,
                             tfrecord=tfrecord,
...
def predict_tile(model,
    ...
    while True:
        try:
            ...
            box_array, score_array, label_array = model.prediction_model.predict_on_batch(
                iterator)
