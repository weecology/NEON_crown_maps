...
#Get site curve
field_density<-field %>% filter(siteID %in% results$Site) %>% filter(!is.na(easting)) %>% filter(stemDiameter > 7.5) %>% filter(is.na(height)|height>3) %>% group_by(site=siteID,plotID) %>% summarize(tree_density=n()) 
  
p + geom_density(data=field_density,aes(x=tree_density),fill="black",alpha=0.5) + theme_bw()
#ggplot(.,aes(x=height)) + geom_density(fill="black",alpha=0.5)
