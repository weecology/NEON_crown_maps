...
field_data<-field %>% filter(height<55, height>3,area>0) %>% group_by(siteID,individualID) %>% summarize(height=mean(height,na.rm=T),area=mean(area,na.rm=T))

ggplot(field_data,aes(x=height,y=log(area))) + geom_point(alpha=0.1)  + stat_smooth(method="lm")

m<-field_data %>% lm(data=.,log(area)~height)
summary(m)

field_predict<-data.frame(x=seq(3,25,1),y=exp(predict(m,newdata = data.frame(height=seq(3,25,1)))))
