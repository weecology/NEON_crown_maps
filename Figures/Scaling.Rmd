...
colnames(allometry)<-c("Year","slope","intercept","min_height","max_height")
...
allometry %>% group_by(Site,Year) %>% do(predict_line(.$min_height,.$max_height,.$intercept,.$slope)) %>% mutate(group=paste(Site,Year)) %>% ggplot(.,aes(x=x,y=y,group=group)) + geom_line(alpha=0.4) + labs(x="Height (m)",y="Crown Area (m^2)") + theme_bw() 
