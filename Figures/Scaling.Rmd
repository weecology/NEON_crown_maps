...
colnames(allometry)<-c("Year","slope","intercept","min_height","max_height","R2","n")
...
field_data<-field %>% filter(height<55, height>3,maxCrownDiameter>0) %>% group_by(siteID,individualID) %>% summarize(height=mean(height,na.rm=T),area=mean(area,na.rm=T))
ggplot(field_data,aes(x=height,y=area,col=siteID)) + geom_point(alpha=0.1)  + stat_smooth(method="lm")

m<-field_data %>% filter(siteID=="DSNY") %>% lm(data=.,area~height)
summary(m)

field_predict<-data.frame(x=seq(3,25,1),y=predict(m,newdata = data.frame(height=seq(3,25,1))))
